...
final class MiniDAWController: NSViewController {
    ...
    override func viewDidLoad() {
        ...
        let explain = NSTextField(wrappingLabelWithString: "Der direkte JUCE-Test und die Plugin-Engine besitzen nicht gleichzeitig das Audiogerät. Beim Laden von Pianoteq wird AudioDeviceCore zuerst geschlossen und das Gerät sauber an die Plugin-Engine übergeben."); explain.textColor = .secondaryLabelColor
    ...
    @objc func loadPianoteq() {
        ...
        if !engineStarted {
            ...
            engineStarted = true; track = md_create_test_session(); findPianoteq()
        }
        var out = [CChar](repeating: 0, count: 512)
        let audioName = md_current_audio_output(&out, 512) ? String(cString: out) : "kein Ausgang"
        engineStatus.stringValue = "Plugin-Engine: bereit · Audio: \(audioName)"
        guard track >= 0 else { status.stringValue = "FEHLER: Testspur konnte nicht erzeugt werden"; return }
        guard audioName != "kein Ausgang" else { status.stringValue = "FEHLER: Plugin-Engine hat kein geöffnetes Audiogerät"; return }
        ...
        status.stringValue = "Pianoteq geladen – jetzt Pianoteq-Ton testen"
        preview.isEnabled = true; play.isEnabled = true; stop.isEnabled = true; load.isEnabled = false
    }

    @objc func previewNote() {
        guard engineStarted, track >= 0 else { return }
        md_preview_note(track, 60, true)
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.8) { [track] in md_preview_note(track, 60, false) }
    }
    ...
    @objc func tick() {
        if engineStarted {
            ...
            audioThreadStatus.stringValue = String(format: "Plugin-Audio: %.3f s", audio)
        }
        if md_direct_audio_running() {
            var b = [CChar](repeating: 0, count: 256); let name = md_direct_audio_device_name(&b, 256) ? String(cString: b) : "?"
            directStatus.stringValue = String(format: "%@ · %.0f Hz · %d smp · %d out · cb %llu · peak %.3f · E %.3f", name, md_direct_audio_sample_rate(), md_direct_audio_buffer_size(), md_direct_audio_active_outputs(), md_direct_audio_callbacks(), md_direct_audio_peak(), md_direct_audio_energy())
        } else { directStatus.stringValue = "AudioDeviceCore: geschlossen" }
    }
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ n: Notification) {
        let vc = MiniDAWController(); window = NSWindow(contentRect: NSRect(x: 0, y: 0, width: 760, height: 500), styleMask: [.titled, .closable, .miniaturizable], backing: .buffered, defer: false)
        window.title = "MiniDAW Functional Test · Build \(miniDAWBuildNumber)"; window.contentViewController = vc; window.center(); window.makeKeyAndOrderFront(nil); NSApp.activate(ignoringOtherApps: true)
    }
