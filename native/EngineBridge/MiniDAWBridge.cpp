...
public:
    bool start() {
        ...
        if (!device) {
            lastError = "JUCE opened no current audio device";
            deviceManager.closeAudioDevice();
            return false;
        }
        sampleRate.store(device->getCurrentSampleRate());
        bufferSize.store(device->getCurrentBufferSizeSamples());
        activeOutputs.store(deviceManager.getAudioDeviceSetup().outputChannels.countNumberOfSetBits());
        deviceName = device->getName();
        if (sampleRate.load() <= 0.0 || activeOutputs.load() <= 0) {
            lastError = "Audio device has no active output channel or sample rate";
            deviceManager.closeAudioDevice();
            return false;
        }
    ...
    void stop() {
        if (running.exchange(false)) deviceManager.removeAudioCallback(this);
        deviceManager.closeAudioDevice();
        remainingSamples.store(0);
    }

    void testTone(double seconds) {
        if (!running.load()) return;
        auto sr = sampleRate.load();
        auto n = static_cast<int64_t>(std::max(0.05, std::min(seconds, 3.0)) * sr);
        remainingSamples.store(n);
    }
    ...
    void audioDeviceIOCallbackWithContext(const float* const*, int,
        ...
        const double step = sr > 0.0 ? juce::MathConstants<double>::twoPi * 440.0 / sr : 0.0;
        float localPeak = 0.0f;
        double localEnergy = 0.0;
        auto remaining = remainingSamples.load(std::memory_order_relaxed);
        ...
        for (int i = 0; i < numSamples; ++i) {
            float sample = 0.0f;
            if (remaining > 0) {
                sample = static_cast<float>(std::sin(phase) * 0.08);
                phase += step;
                if (phase >= juce::MathConstants<double>::twoPi) phase -= juce::MathConstants<double>::twoPi;
                --remaining;
            }
            localPeak = std::max(localPeak, std::abs(sample));
            localEnergy += static_cast<double>(sample) * sample;
            for (int ch = 0; ch < numOutputs; ++ch)
                if (outputs[ch]) outputs[ch][i] = sample;
        }
        remainingSamples.store(remaining, std::memory_order_relaxed);
        peak.store(std::max(peak.load(std::memory_order_relaxed), localPeak), std::memory_order_relaxed);
        energy.store(energy.load(std::memory_order_relaxed) + localEnergy, std::memory_order_relaxed);
    }

    void audioDeviceAboutToStart(juce::AudioIODevice* d) override {
        if (d) {
            sampleRate.store(d->getCurrentSampleRate());
            bufferSize.store(d->getCurrentBufferSizeSamples());
        }
    }
    ...
    std::atomic<int> activeOutputs{0};
...
extern "C" {
MD_EXPORT bool md_direct_audio_start() {
    if (!directAudio) directAudio = std::make_unique<AudioDeviceCore>();
    return directAudio->start();
}
...
MD_EXPORT int md_create_test_session() {
    ...
    int track = tm.createTrack("MiniDAW Test"); if (track < 0) return -1;
    int clip = cm.createMidiClipBeats(track, 0.0, 8.0); if (clip < 0) return -2;
    if (auto* c = cm.getClip(clip)) c->name = "Vier Noten";
    cm.addMidiNote(clip, {60,100,0.0,0.8}); cm.addMidiNote(clip, {64,100,1.0,0.8});
    cm.addMidiNote(clip, {67,100,2.0,0.8}); cm.addMidiNote(clip, {72,100,3.0,0.8});
...
MD_EXPORT bool md_select_audio_output(int index) {
    auto* e=magda_get_engine(); auto* dm=e?e->getDeviceManager():nullptr; auto* type=currentAudioType(); if(!dm||!type)return false;
    auto names=type->getDeviceNames(false); if(index<0||index>=names.size())return false;
    auto setup=dm->getAudioDeviceSetup(); setup.outputDeviceName=names[index]; auto result=dm->setAudioDeviceSetup(setup,true);
    if(!result.isEmpty()){ setup.inputDeviceName=names[index]; result=dm->setAudioDeviceSetup(setup,true); } if(!result.isEmpty())return false;
    if(auto* device=dm->getCurrentAudioDevice()){ juce::BigInteger outputs; outputs.setRange(0,device->getOutputChannelNames().size(),true); auto active=dm->getAudioDeviceSetup(); active.outputChannels=outputs; if(!dm->setAudioDeviceSetup(active,true).isEmpty())return false; e->setEnabledWaveChannels(false,outputs); }
    return true;
}
}
